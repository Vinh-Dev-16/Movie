...
$rowsPerPage = 15;
$perRow = ($page - 1) * 15;
$query4 = "SELECT * FROM movie,genre where movie.genre_id=genre.id  LIMIT $perRow,$rowsPerPage";
$run4 = mysqli_query($con, $query4);
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $query1 = "SELECT * FROM genre where id = $id";
                                ...
                                <section>
                                    <div class="genre">
                                        <div class="name_genre">
                                            <h3><?php echo $row1['genre_name'] ?></h3>

                                        </div>
                                        <div class="mv_genre">
                                            <?php
                                            $query = "SELECT * FROM genre,movie where genre.id=movie.genre_id and genre.id=$id ORDER BY mv_date DESC";
                                    ...
                                    <ul class="pagination">
                                        ...
                                        <?php
                                        $id = $_GET['id'];
                                        $query3 = "SELECT * FROM genre,movie where genre.id=movie.genre_id and genre.id=$id ";
                                        $run3 = mysqli_query($con, $query3);
                                        $total = mysqli_num_rows($run3);
                                        $total_page = ceil($total / $rowsPerPage);

                                        if ($page > 1) {
                                            echo "<li ><a class='prev' href='viewgenre.php?id=$id&page=" . ($page - 1) . "'  >Prev</a></li>";
                                        } else {
                                            echo "<li ><a class='prev' href='viewgenre.php?id=$id&page=" . ($total_page) . "'  >Prev</a></li>";
                                        }
                                        for ($i = 1; $i <= $total_page; $i++) {
                                            if ($i  > $page - 3 && $i < $page + 3) {
                                                echo "<li ><a class='pageNumber' href='viewgenre.php?id=$id&page=" . ($i) . "'  >$i</a></li>";
                                            }
                                        }

                                        if ($total_page > $page) {
                                            echo "<li ><a class='next' href='viewgenre.php?id=$id&page=" . ($page + 1) . "'  >Next </a></li>";
                                        } else {
                                            echo "<li ><a class='next' href='viewgenre.php?id=$id&page=1'  >Next </a></li>";
                                        }
