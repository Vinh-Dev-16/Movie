  ...
  <div class="wrapper_res">
    <div class="main">
      ...
      <form action="" method="POST" class="form" id="form-1" role="form">
        ...
        <div class="form-group">
          <input id="password" required placeholder=" " name="pwd" type="password" class="form-control" value="">
...
if (isset($_POST['submit'])) {
  ...
  $user = 'uname' . $uname;
  if ($pwd !== $repwd) {
    header("Location: registeradmin.php?error=Hai password nhập không trùng nhau&$user");
    exit();
  } else if (strlen($pwd) < 8) {
    header("Location: registeradmin.php?error=Mật khẩu phải nhập trên 8 kí tự&$user");
    exit();
  }
  $sql = "SELECT * FROM `account` WHERE uname = '$uname' ";
  $run = mysqli_query($con, $sql);
  if (mysqli_num_rows($run) > 0) {
    header("Location: registeradmin.php?error=Username này đã tồn tại&$user");
    exit();
  } else {
    $query = "INSERT INTO `account`(`uname`, `pwd` ,`role`,`accdate`) VALUES ('$uname', '$hash',1,'$date')";
    $save = mysqli_query($con, $query);
    if ($save) {
      header("Location: registeradmin.php?success=Đã đăng ký thành công");
      exit();
    } else {
      header("Location: registeradmin.php?error= Đã xảy ra lỗi");
    }
  }
}
?>